...
export interface IBsdMod<T, R> {
    decode(value: T, reader: BsdReader): R;
}
...
export interface BsdDecoder<T, C> {
    readonly config: C;
    decode(reader: BsdReader): T;
}

/** Any BSD schema. */
export type BsdAny = Bsd<any>;
...
export interface Bsd<T> {
     ...
     * ```
     */
    pad(bytes: number | BsdMod<T, number>): any;
     ...
     * ```
     */
    is<const U extends T>(expected: U): any;
     ...
     * ```
     */
    in<const U extends T>(values: Array<U> | Set<U> | Map<U, unknown>): any;
     ...
     * ```
     */
    pipe<const S extends BsdAny>(pipe: BsdMod<T, S> | S): any;
...
export interface BsdBytes extends Bsd<Uint8Array> {
     ...
     */
    frame<const S extends BsdAny>(schema: S | BsdMod<Uint8Array, S>): BsdFor<S>;
...
export interface BsdStruct<T extends BsdShape> extends Bsd<T> {
    /**
