...
describe("EkeReader", () => {
	test.each([
		...
		const reader = new EkeReader(Uint8Array.from(bytes));
	...
	test("scopes DataView reads to Uint8Array subarrays", () => {
		...
		const reader = new EkeReader(backing.subarray(2));
	...
	test("reads ASCII by byte count and rejects invalid counts", () => {
		const reader = new EkeReader(Uint8Array.from([65, 66]));

		expect(reader.ascii(0)).toBe("");
		expect(reader.ascii(2)).toBe("AB");
		expect(new EkeReader(Uint8Array.of(65)).ascii(1)).toBe("A");
		const backing = Uint8Array.from([0, 0x80, 0xff]);
		expect(new EkeReader(backing.subarray(1)).ascii(2)).toBe("\x80\xff");
		for (const bytes of [-1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
			expect(() => new EkeReader(Uint8Array.of(65)).ascii(bytes)).toThrow(EkeError);
		}
	});
	...
	test("reads UTF-8 by byte count", () => {
		...
		const reader = new EkeReader(backing.subarray(1));
		...
		for (const bytes of [-1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
			expect(() => new EkeReader(Uint8Array.of()).utf8(bytes)).toThrow(EkeError);
		}
	});
	...
	test("reads little-endian UTF-16 by byte count", () => {
		...
		for (const bytes of [-2, 1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
			expect(() => new EkeReader(Uint8Array.of()).utf16le(bytes)).toThrow(EkeError);
		}
	});

	test("rejects invalid offsets and reads beyond either boundary", () => {
		for (const offset of [-1, 0.5, Number.NaN]) {
			const reader = new EkeReader(Uint8Array.of(1));
			reader.offset = offset;
			expect(() => reader.uint(8)).toThrow(EkeError);
		}

		const framed = new EkeReader(Uint8Array.of(1, 2));
		framed.limit = 1;
		expect(() => framed.uint(16)).toThrow(EkeError);

		const oversizedLimit = new EkeReader(Uint8Array.of(1));
		oversizedLimit.limit = 2;
		expect(() => oversizedLimit.uint(16)).toThrow(EkeError);
	});

	test("every reader enforces its inlined bounds check", () => {
		const reads: Array<(reader: EkeReader) => unknown> = [
		...
		for (const read of reads) {
			expect(() => read(new EkeReader(Uint8Array.of()))).toThrow(EkeError);
		}
	});
});
