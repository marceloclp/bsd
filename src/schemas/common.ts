import { BsdType, type BsdAny, type BsdDecoder, type BsdFor, type BsdInfer, type BsdNumber } from "../bsd";
...
/**
 * Produces a value without consuming bytes.
 *
 * This is particularly useful when defining a schema brand/type inside unions,
 * as it allows for type-narrowing when checking the output.
 *
 * @example
 *     ```typescript
...
/**
 * Produces a dynamic two-step schema.
 *
 * The first step decodes a runtime-resolved value schema
 * and uses its value to build the schema that consumes the
 * following bytes.
 *
 * This is useful when decoding variable-length data.
 *
 * @example
 * ```typescript
...
export function eager<T extends BsdAny, T1 extends BsdAny>(
    ...
    return BsdType.make((reader) => {
        let v: any = typeof value === "function"
 ...
 /**
  * Finds the offset BEFORE the first occurence of a value in the
  * buffer that satisfies the given predicate.
  *
  * This is useful when searching for the end offset of a variable-length
  * sequence (e.g., a null terminated string).
  *
  * This schema does not consume the buffer.
  *
  * @example
  * ```typescript
  ...
  * ```
  */
export function find<S extends BsdAny>(schema: S, is: (value: BsdInfer<S>) => boolean) {
    return BsdType.make((reader) => {
 ...
 /**
  * Skips the first `n` bytes, as they represent padding
  * between two values, usually due to alignment or because
  * one is variable-length.
  *
  * @example
  * ```typescript
  ...
  * ```
  */
export function padded<const S extends BsdAny>(byteLength: number, schema: S): BsdFor<S> {
    return BsdType.make((reader) => {
